...
| **Language** | C | Rust | **19 `unsafe` blocks in the module**, all POSIX calls with no Rust equivalent and each carrying a `SAFETY` note: 10 in `xattr_fs.rs` (the `l*xattr` family), 5 in `acl_fs.rs` (`acl_get_fd` / `acl_set_fd` / `PosixACL::from_raw`), 3 in `delta_transport_impl.rs` (`utimensat`, `getpwuid_r`, `getgrgid_r`), and 1 in `streaming_writer.rs` (`fchmod` on the still-open temp fd before ACL apply). No pointer arithmetic, no hand-rolled parsing in unsafe: every wire byte is decoded in safe Rust. This row used to read "no unsafe in the module", which was measurably wrong, and a memory-safety claim is the last place to round a number |
| **License** | GPL-3.0-or-later (`rsync.h`: version 3, or any later version) | `MPL-2.0 OR GPL-3.0-or-later` | Stated in `src-tauri/src/aerorsync/LICENSING.md` and as an SPDX tag on every Rust file in the module. The old cell said GPL-3.0-only and called that aligned with AeroFTP. AeroFTP is GPL-3.0-or-later, and the module is the dual expression above |
...
| **Wire protocols supported** | 27, 28, 29, 30, 31, 32 with negotiation | 31/32 only (subset of 31, encode/decode 31/32) | rsync must dialog with ancient installs. AeroRsync speaks to rsync 3.1.x as well as 3.2.x / 3.4.x (see the row on peers without negotiated strings); a protocol-30 peer (rsync 3.0.x) is refused before any data moves and the file goes over a plain SFTP transfer without delta, which is the fallback on every platform once the native path is the delta transport. The refusal is remembered for thirty minutes per host, port and user, so a sync batch against such a server opens one refused session, not one per file |
...
| **File-level checksum** | MD5 (modern default) / xxh3 / xxh128 / MD4 (legacy) | Negotiated: xxh128, xxh3, xxh64, md5, md4 (advertised in that order), plus sha1 reachable via `AEROFTP_RSYNC_CSUM_ALGOS`; MD5, the protocol default, with a peer that negotiates nothing | The whole-file trailer is computed and **verified** for every implemented winner, in both directions; all six are covered live against a real peer. An unsupported or `none` winner is rejected pre-commit (`NegotiationFailed`) and cannot disable verification in the integrated product. Checksum `none` as an unverified transfer mode remains a later explicit design decision for a standalone crate, not for AeroFTP. **sha1 depends on the peer's vintage**: rsync 3.2.7 advertises `xxh128 xxh3 xxh64 md5 md4 sha1 none`, rsync 3.4.1 advertises the same list **without sha1** - upstream dropped it. Forcing sha1 against 3.4.1 gets `requested action not supported (code 4) at compat.c(405)`, which is the peer refusing, not AeroRsync failing |
...
| **Literal compression** | Advertises `zstd lz4 zlibx zlib none` | Advertises **`zstd zlibx none`**, and drives every name it advertises. Multi-chunk DEFLATED_DATA splitting >16 KiB | The winner is the first name in **our own** list the peer also advertises, exactly as `compat.c::parse_negotiate_str` computes it, which makes the advertisement a promise rather than a wish list: mirroring stock's list verbatim meant a peer built with `lz4` but without zstd handed us `lz4`, for which there is no codec here at all, while `zlibx` sat one position lower, implemented and measured. `none` is listed last so the two lists always intersect and a peer offering only codecs we decline lands on uncompressed literals, which is a working delta transfer, instead of an empty intersection. zstd mirrors `token.c::send_zstd_token`; `zlibx` is raw deflate on one session-wide stream with the four-byte `Z_SYNC_FLUSH` tail the sender strips and the receiver re-appends, pinned against a captured **rsync 3.1.3** wire oracle that runs in CI. Both families share the same outer `TOKEN* / DEFLATED_DATA / END` grammar; only the payload codec and its flush boundaries differ |
| **Why not plain `zlib`** | Its own default before 3.2.0 | Declined, and structurally rather than for effort | `zlib` feeds matched block data through the compressor history on both ends (`token.c::see_deflate_token`), so the decoder needs `inflateIncomp`, a function rsync patches into the zlib it bundles. It is in neither the system zlib nor any Rust crate, which is why upstream split `zlibx` out in the first place: the rsync man page describes zlibx as zlib "with matched data excluded from the compression stream (to try to make it more compatible with an external zlib implementation)". Supporting it would mean owning an inflate implementation, so instead we never negotiate it |
| **Peers without negotiated strings (rsync 3.0.x / 3.1.x)** | The client reads the server's compat flags first and exchanges algorithm lists only if they carry `CF_VARINT_FLIST_FLAGS`; otherwise MD5 and classic flag bytes, and `-z` means legacy `zlib` | Same order, measured against rsync 3.1.3: the protocol version alone, then the compat flags, then lists only when negotiated. Without them: MD5 for block sums and the whole-file trailer (verified), classic one- or two-byte file-list flags, and `--new-compress` on the server argv so compression is `zlibx` rather than `zlib`. An endpoint is remembered per host, port and user, so only its first session pays the reopen with `--new-compress`; should a remembered endpoint fail under `--new-compress`, the entry is dropped and the session retried once with the original argv | Until 2026-09-24 the client sent its lists together with its version; rsync 3.1.3 read them as data and exited with code 12, and the fallback, a plain SFTP transfer without delta, hid it. Both directions now run natively and are exercised in CI against a rsync 3.1.3 server |
| **Literal compression: `lz4`** | Supported when built with liblz4 | Not implemented, and **not advertised** | Previously advertised and ranked above `zlibx` by stock's ordering, which is the shape of the bug described above |
...
| **Preserved metadata** | perms, owner, group, mtime, atime, symlinks, hardlinks, xattrs, ACL, sparse, devices | mtime, base perms via atomic finalize, **symlinks on unix** (single-file, both directions), **`user.*` xattrs on Unix** (`-X`), and **POSIX access ACL on Linux** (`-A`), all against stock rsync; **no owner/group/device/special files** | Production sends `-ltprcz` (plus `A`/`X` when those metadata bits are on), never `-o -g -D`. File-list entries therefore omit uid/gid. A source symlink is detected by `lstat` and never followed, emitted as an `S_IFLNK` file-list entry carrying the target; the receiver materialises it create-temp-then-rename. Linux production opts into `-A`; Unix production opts into `-X`. ACL reads and applies use the already-open fd and never follow a symlink. **Directory default ACL awaits recursive entries.** Hardlink remains structurally blocked on recursive scope. See the taxonomy below |
...
| **Tests against real rsync servers** | Community test suite + buildbot | **698** executed tests (713 selected, 15 ignored) against **rsync 3.2.7 frozen bytes**; **15** CI lane 3 live tests vs stock **rsync 3.2.7** in Docker (gated `RUSTFLAGS='--cfg ci_lane3'`), including byte-identical upload, bidirectional named-user ACL preservation, and unsupported checksum fallback; 8 live tests across the negotiated checksum matrix driving the production transports; 3 `#[ignore]` comparative benchmarks | Measured on `main` `c713857e` (#935): 21 new unit tests since `4ab49265`, none ignored. `.github/workflows/aerorsync-protocol.yml` still fails the job below 677. Blocco B closed 2026-04-26: production dispatch uses stock `rsync --server` (`RemoteCommandFlavor::WrapperParity`) |
| **Default enabled in AeroFTP** | N/A (AeroFTP does not spawn it in production) | **Yes, mode `auto` since v3.8.0**. Native is attempted first. The stock `rsync` binary replaces it only on Unix, and only when the native transport cannot be constructed or the parent session did not pin a host key. A native session that then fails does not start that binary: the file is a plain SFTP transfer, with no delta, on every platform. Hard refusals (host-key mismatch and the other non-transient protocol faults) are shown and not retried | Mode `native` never uses the stock binary. Mode `classic` is the binary on purpose. The cross-OS host-key asymmetry that previously kept the toggle off was resolved in May 2026 |
...
| **Identity in AeroFTP code** | Invoked by `rsync_over_ssh.rs` (1299 lines, `RsyncBinaryTransport`) | Module `src-tauri/src/aerorsync/` (25 Rust files, ~44 800 lines total, `AerorsyncDeltaTransport` impl `DeltaTransport`) | Both implement the same `DeltaTransport` trait, runtime choice |
...
| Checksum `none` (unverified transfer) | rejected pre-commit in the integrated product | **later explicit design decision** |
...
- **The compatibility surface to protocols 27-30.** AeroRsync is protocol-31-only by design. With the native path (rsync mode `auto`, the default, or `native`) an older or stripped endpoint is refused before any data moves and the file goes over a plain SFTP transfer without delta, on every platform; only the `classic` mode, Unix only, serves such an endpoint with the stock rsync binary. A frame that is actually corrupt remains a hard error and is shown.
...
1. **owner / group (`-o` / `-g`)** - the integrated product profile does not request them and does not emit uid/gid on the file list. `StreamingAtomicWriter::finalize` applies mode, mtime, Unix `user.*` xattrs and Linux access ACLs. Applying owner/group needs a privileged receiver, which rarely matches AeroFTP's desktop deployment model. Historical capture literals still carry `-o -g -D` for frozen oracles only.
2. **Device and special files** - Unix-only, needs a privileged create. Production does not send `-D`.
...
Items that have already left this list: **symlinks** ship end-to-end on unix in both directions against stock rsync (Y-RSC.4, single-file scope); **sparse files** have an opt-in `--sparse` analogue on the local delta path; **`user.*` xattrs (`-X`)** ship on Unix single-file against stock rsync; and **POSIX access ACL (`-A`)** ship on Linux single-file in both directions, with fd-based read/apply and Linux-only production opt-in. Named user 65534 plus an explicit mask are covered live in both directions. **Directory default ACL awaits recursive entries.** Windows and macOS stay ACL-off; destination ENOTSUP is soft by default while source ACL read failures are hard.
...
- **Memory-safe** Rust implementation: every wire byte is decoded in safe Rust, so the buffer-overflow class that comes with parsing untrusted input in C is absent by construction. The 19 `unsafe` blocks in the module are POSIX calls with no Rust equivalent (xattr, ACL fd ops, `fchmod`, `utimensat`, user and group lookup), not parsing
...
- Compress literals (rsync advertises `zstd lz4 zlibx zlib none`, AeroRsync advertises `zstd zlibx none` and drives all three)
...
- Share a GPL-3.0-or-later option. Upstream rsync states it in `rsync.h`. AeroRsync states `MPL-2.0 OR GPL-3.0-or-later`, so a recipient can take the module under either licence. The application that compiles the module in is GPL-3.0-or-later
...
| Preserve ACL/xattr/owner | Does it with `-aHAX` | **xattr (`user.*`, `-X`) on Unix** and **POSIX access ACL (`-A`) on Linux**: yes, single-file both directions. Directory default ACL awaits recursive entries. **owner/group**: deliberately neither requested nor applied by the integrated desktop product (needs a privileged receiver) |
...
Loopback comparison against stock rsync 3.2.7, 28 July 2026, idle 24-core machine, SSH on loopback, 50 MB fixtures. Both sides used the server argument string AeroRsync actually sends (`--server -logDtprcze.iLsfxCIvu`), checked by logging the command line inside the container. The everyday `-az` was not used: the two fixtures share a size and an mtime, so rsync's quick check would skip the transfer. Three cold-upload runs read 1.005 s, 1.049 s and 1.068 s. Byte counts were identical on every run. The table uses 1.068 s.

![AeroRsync and stock rsync on 50 MB fixtures, wall-clock seconds](images/aerorsync-vs-rsync-2026-07-28.png)
...
| Delta upload, 640 times 4 KiB regions changed | 2.000 s | 1.358 s | 3 748 322 vs 3 743 589 |
...
| 20 times 256 KiB, one session per file | 4.981 s | 25.249 s | |
| 20 times 256 KiB, one recursive rsync call | no recursive scope | 1.308 s | |

![Twenty files of 256 KiB, per-file sessions against one recursive rsync call](images/aerorsync-smallfiles-2026-07-28.png)

Reading the same runs:

1. AeroRsync is faster where the other side pays for a new process. Cold upload, the no-op, and twenty small files one at a time. Stock rsync pays a fresh `ssh` plus `rsync` fork per file, about 1.25 s each. AeroRsync opens an in-process session. That is the 5.1 times on the small-file batch and the 2.7 times on the no-op.
2. On the delta upload AeroRsync is slower, about 2.000 s against 1.358 s, while the byte counts agree to 0.13 percent (3 748 322 against 3 743 589). The protocol decisions match. The gap is CPU in the Rust encode path.
3. The row with no AeroRsync number is the scope gap. One recursive rsync invocation moves the same twenty files in 1.308 s. AeroRsync does not own the tree. `AerorsyncBatch` reuses one SSH session for many files, and that path is not what this table measures.

The delta fixtures compress. `A_base.bin` compresses 9.1:1, so a real-world delta on incompressible data will move more bytes. Both engines saw the same bytes, so the comparison stands. The cold-upload fixture is incompressible (1.0:1).
...
3. **Old or stripped-rsync endpoints**: an endpoint whose `rsync --server` negotiates an old wire protocol (27-30) or whose wrapper rejects the standard server flag string is **out of scope** for the native engine, which is protocol-31-only by design. With the native path (rsync mode `auto`, the default, or `native`) such an endpoint is refused before any data moves and the file goes over a plain SFTP transfer without delta, on every platform. A frame that is actually corrupt remains a hard error and is shown. Delta against an old endpoint needs the `classic` mode, which runs the stock `rsync` binary and exists on Unix only. A rsync 3.1.x peer negotiates no algorithm strings and is spoken natively.
...
**Fairness anchor (historical).** The 2026-07-28 numbers used rsync `-logDtprcz`, which produces `--server -logDtprcze.iLsfxCIvu`. That was the AeroRsync capture profile at the time, not the current integrated product profile. Current production sends `-ltprcz` (plus `A`/`X` when those bits are on): `--server -ltprcze.iLsfxCIvu`. These timings were not remeasured with the product flags. The everyday `-az` set was deliberately *not* used for the published numbers, because with it the delta scenarios measure nothing at all - the two 50 MB fixtures share a size and an mtime, so rsync's quick check declares them identical and skips. The `c` (`--checksum`) in the strict set is what forces a real comparison.
...
The integrated desktop product has two explicit metadata non-goals: owner/group and device/special files require receiver privileges that do not fit its deployment model, so production neither requests nor emits them. Hardlinks and directory default ACL are different: both require recursive, multi-entry session state and remain gates for that future session model and for the later standalone-crate evaluation, not promises of the current single-file path. **`user.*` xattrs (`-X`) ship on Unix** and **POSIX access ACL (`-A`) ship on Linux**. Symlinks (v4.1.6), xattrs (B1-B4) and ACL (B1-B4, 2026-08-27) are the worked examples of how a metadata type gets added end-to-end here.
...
- "Works wherever rsync works": no daemon mode, no recursive tree, no owner/device/hardlink metadata, and protocol-31 only (an endpoint whose `rsync --server` negotiates protocol 27-30, or whose wrapper rejects the standard server flags, is out of scope: the native path moves the file over a plain SFTP transfer without delta there, and delta against it needs the `classic` mode with the stock rsync binary, Unix only). Unix preserves `user.*` xattrs via `-X`; Linux preserves POSIX access ACL via `-A`. Directory default ACL awaits recursive entries. Password auth and SSH agent auth (Unix) are wired into the native transport
...
- "AeroRsync lacks `--delete`" (and the same for filters, `--inplace`, daemon mode): true as a sentence, misleading as a gap. These are handled at another layer on purpose, and saying it the other way invites the reader to score a design decision as a missing feature. Say what AeroFTP *does*: "deletion happens at the sync layer, with its own safety gates". Apply the same discipline to metadata: owner/group and device/special files are integrated-product non-goals, while hardlinks and directory default ACL are future multi-entry/standalone gates (`user.*` xattr on Unix and POSIX access ACL on Linux already ship)

---

## One line per audience

- **For Rust developers**: "Clean-room Rust impl of rsync wire protocol 31, single-file delta over SSH, in-process inside AeroFTP, MPL-2.0 OR GPL-3.0-or-later."
...
- **For partners (Hetzner, rsync.net, Filen)**: "The native AeroRsync module speaks wire-31 byte-identical with rsync 3.2.7 and is in production single-file inside AeroFTP for cross-OS coverage, mode `auto` (native first, classic fallback) by default since v3.8.0. Key-based endpoints (rsync.net, Hetzner) work today through the SFTP provider's delta path, including SSH agent identities on Unix, and password-backed SFTP profiles enter the native transport directly. A rsync 3.1.x peer negotiates no algorithm strings and is spoken natively. Endpoints that run an old or stripped `rsync --server` (protocol 27-30) fall outside the protocol-31 native engine: with the native path they are refused before any data moves and the file goes over a plain SFTP transfer without delta, and delta only through the stock rsync binary when the rsync mode is set to `classic` (Unix only)."

---

*Last updated: 2026-09-24 (legacy peers): the native path speaks to rsync 3.1.x servers, which negotiate no algorithms, in both directions (MD5, classic flag bytes, zlibx via `--new-compress`), live in CI against rsync 3.1.3; protocol-30 peers are refused before any data moves. Previously 2026-08-27 (B4 ACL closeout): POSIX access ACL (`-A`) ship on Linux single-file in both directions against stock rsync 3.2.7; Linux production opts in via `SftpProvider::delta_transport`; Debian and Snap carry the `libacl1` runtime; the two ACL live tests are hard-wired into CI lane 3. Directory default ACL awaits recursive entries. Previously 2026-07-27 (B4 xattr closeout): `user.*` xattrs (`-X`) shipped on Unix single-file and Unix production opted into `-X`. Updated when the AeroRsync module reaches new milestones (recursive scope expansion, filters at wire level).*
