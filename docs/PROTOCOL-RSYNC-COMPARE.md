...
- **The compatibility surface to protocols 27-30.** AeroRsync is protocol-31-only by design. An older or stripped peer is outside that engine. A native session that stops on it does not hand the file to the stock rsync binary: the file is a plain SFTP transfer.
...
3. **Old or stripped-rsync endpoints**: an endpoint whose `rsync --server` negotiates an old wire protocol (27-30) or whose server rejects the standard flag string is **out of scope** for the native engine, which is protocol-31-only by design. If a native session was built and then stops, the file is a plain SFTP transfer. The stock `rsync` binary is not started for that file. It is used only on Unix, and only when native was never built or the host key was never pinned.
