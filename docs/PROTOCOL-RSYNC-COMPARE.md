...
| **Tests against real rsync servers** | Community test suite + buildbot | 657 unit tests against **rsync 3.2.7 frozen bytes**; 14 CI lane 3 live tests vs stock **rsync 3.2.7** in Docker (gated `RUSTFLAGS='--cfg ci_lane3'`), including byte-identical upload and bidirectional named-user ACL preservation; 8 live tests across the negotiated checksum matrix driving the production transports; 3 `#[ignore]` comparative benchmarks | Blocco B closed 2026-04-26: production dispatch uses stock `rsync --server` (`RemoteCommandFlavor::WrapperParity`) |
