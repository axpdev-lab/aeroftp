...
| **Literal compression** | zlib (default), zlibx (`--zc=zlibx`), zstd (`--zc=zstd`), none (`-z` off) | zstd and **zlibx** (raw deflate), multi-chunk DEFLATED_DATA splitting >16 KiB; plain `zlib` and `lz4` fall back | The winner is derived the way `compat.c::parse_negotiate_str` does, not by asking whether the peer merely *advertised* zstd, which is the defect that shipped until v4.1.7. Both families use the same outer `TOKEN* / DEFLATED_DATA / END` grammar; only the payload codec and its flush boundaries differ. zstd mirrors `token.c::send_zstd_token`; `zlibx` is raw deflate on one session-wide stream with the four-byte `Z_SYNC_FLUSH` tail the sender strips and the receiver re-appends, pinned against a captured **rsync 3.1.3** wire oracle that runs in CI. Plain `zlib` additionally runs matched-block data through the compressor history on both ends (`token.c::see_deflate_token`), which is not modelled, so it takes a typed `NegotiationFailed` and the classic fallback rather than a wrong reconstruction |
...
- Compress literals (rsync = zlib/zlibx/zstd, AeroRsync = zstd and zlibx; plain zlib falls back)
