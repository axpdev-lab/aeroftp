...
| **File-level checksum** | MD5 (modern default) / xxh3 / xxh128 / MD4 (legacy) | Negotiated: xxh128, xxh3, xxh64, md5, md4 (advertised in that order), plus sha1 reachable via `AEROFTP_RSYNC_CSUM_ALGOS` | The whole-file trailer is computed and **verified** for every implemented winner, in both directions; all six are covered live against a real peer. An unsupported or `none` winner is rejected pre-commit (`NegotiationFailed`) and cannot disable verification in the integrated product. Checksum `none` as an unverified transfer mode remains a later explicit design decision for a standalone crate, not for AeroFTP. **sha1 depends on the peer's vintage**: rsync 3.2.7 advertises `xxh128 xxh3 xxh64 md5 md4 sha1 none`, rsync 3.4.1 advertises the same list **without sha1** - upstream dropped it. Forcing sha1 against 3.4.1 gets `requested action not supported (code 4) at compat.c(405)`, which is the peer refusing, not AeroRsync failing |
...
| **Tests against real rsync servers** | Community test suite + buildbot | CI floor of **677** executed tests (692 selected, 15 ignored) against **rsync 3.2.7 frozen bytes**; **15** CI lane 3 live tests vs stock **rsync 3.2.7** in Docker (gated `RUSTFLAGS='--cfg ci_lane3'`), including byte-identical upload, bidirectional named-user ACL preservation, and unsupported checksum fallback; 8 live tests across the negotiated checksum matrix driving the production transports; 3 `#[ignore]` comparative benchmarks | The 677 floor is what `.github/workflows/aerorsync-protocol.yml` asserts on `main` at `4ab49265`. Blocco B closed 2026-04-26: production dispatch uses stock `rsync --server` (`RemoteCommandFlavor::WrapperParity`) |
...
- **The compatibility surface to protocols 27-30.** AeroRsync is protocol-31-only by design. A received protocol 27, 28 or 29 is `InvalidProtocolVersion`, mapped to `InvalidFrame`, and the fallback policy treats that as a hard error: the transfer stops and is shown, with no plain SFTP retry and no stock `rsync` binary. Protocol 30 is inside the version window the decoder accepts on this main. #935 is still open, so this page does not describe that peer.
...
3. **Old or stripped-rsync endpoints**: the native engine is protocol-31-only. A received protocol version 27, 28 or 29 is rejected while reading the server preamble (`InvalidProtocolVersion`, mapped to `InvalidFrame`). That is a hard error: the transfer stops and is shown. There is no plain SFTP retry, and the stock `rsync` binary is not started for that file. Protocol 30 is inside the version window this main accepts at the decoder. #935 is still open, so this page does not describe that peer. The stock binary remains only the Unix stand-in when the native transport was never built, or when the parent session never pinned a host key.
...
- "Works wherever rsync works": no daemon mode, no recursive tree, no owner/device/hardlink metadata, and protocol-31 only. A received protocol 27-29 stops and is shown (`InvalidFrame`): AeroFTP does not switch that file to plain SFTP or to the stock binary. Protocol 30 stays inside the decoder window on this main, and #935 is still open. Unix preserves `user.*` xattrs via `-X`; Linux preserves POSIX access ACL via `-A`. Directory default ACL awaits recursive entries. Password auth and SSH agent auth (Unix) are wired into the native transport
