...
| v4.0.9 | **AeroVault Zip plaintext lane (`.aerozip`)** | A fast, honestly-unencrypted archive format with optional Reed-Solomon recovery parity (opt-out, `--recovery-level 0` to disable), a real measured size estimate, CLI (`aeroftp aerovz`) and a GUI scaffold, opened by header on double-click with the OS file-type association |
| v4.0.9 | **AeroMount read-only mount + Save-All** | Mount an unlocked Cryptomator vault, `.aerovault` or `.aerozip` as a read-only filesystem, or export its whole decrypted tree in one shot to a folder, a `.zip` or a `.aerozip`; the password is passed over stdin and never stored, and the mount auto-unmounts on lock or quit |
| v4.0.9 | **AeroVault create redesign** | Compressor-style create: a required Vault Name, mode cards with a conditional password, a result receipt, and a tabbed Home/Recent/Files shell shared by the standalone modal and the browser |
| v4.0.8 | **Change Mode (v2 <-> v3 repack)** | Re-pack an open vault between the v2 and v3 formats under a new security level, keeping the same password; available next to Change Password in the GUI and as `aeroftp vault change-mode` in the CLI |
