...
| v4.0.9 | **AeroVault Zip plaintext lane (`.aerozip`)** | A fast, honestly-unencrypted archive format with optional Reed-Solomon recovery parity (opt-out, `--recovery-level 0` to disable), a real measured size estimate, CLI (`aeroftp archive`) and a GUI scaffold, opened by header on double-click with the OS file-type association |
