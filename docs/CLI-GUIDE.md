...
| `--sftp-readahead <N>` | SFTP read-ahead window (2 to 1024). Default 32 when unset, which keeps up to 32 reads in flight on one connection; `AEROFTP_SFTP_READAHEAD=off` restores one read per round trip; a bandwidth cap always uses the serial loop |
