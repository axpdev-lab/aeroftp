...
# `readme` nel manifest punta a un file che deve esistere nel pacchetto: un
# campo che nomina un file assente e' peggio del campo assente, perche'
# `cargo package` lo rifiuta solo al momento della pubblicazione.
...
# La MSRV segue la stessa regola delle versioni: letta dal manifest
# dell'applicazione, non ricopiata qui. Senza questo campo un consumatore su un
# toolchain piu' vecchio riceve un errore di compilazione dentro il crate
# invece della frase che nomina la versione richiesta.
RUST_VERSION="$(sed -n 's/^rust-version = "\(.*\)"$/\1/p' "$APP_MANIFEST" | head -1)"
[ -n "$RUST_VERSION" ] || { echo "rust-version non trovata in $APP_MANIFEST" >&2; exit 1; }
...
readme = "README.md"
keywords = ["rsync", "sync", "delta", "ssh", "protocol"]
