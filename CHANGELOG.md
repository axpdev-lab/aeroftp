...
**CLI `vault` subcommand (v3)**. `aeroftp-cli vault` exposes the v3 lifecycle (create, open, add, extract, list, info) so the same engine the GUI uses is scriptable and round-trip verifiable from CI. `add --receipt` prints the technical receipt to stdout. The library crate gained the necessary `pub` surface for the binary to call the v3 engine directly. v1/v2 CLI parity is staged but not in this release.
