...
| AeroVault v3 containers (Published, v3.8.0) | Gear-CDC chunking + zstd per chunk + AES-256-GCM-SIV per chunk + BLAKE3-128 chunk id + BLAKE3-256 cipher hash + Argon2id KEKs (HKDF + AES-KW) + HMAC-SHA512 header | Published format, opt-in via the Archive tier; reserves an extension area for the v4 EC layer (non-critical) |
| AeroVault v4 + EC (T-AEROVAULT-ECC, shipped) | v3 + non-critical "error-correction.reed-solomon" extension (Reed-Solomon 10+2 on the ciphertext live-block stream, v2 fixed-grid ~20% overhead, per-shard 16B BLAKE3 cksums, all-or-nothing repair gate) | "v3 + EC = v4" forward-compat (pure v3 open/extract still works); EC-last wrapper per #272/#276; scrub/repair operational; telemetry in receipts. See APPENDIX-AEROVAULT-V4-ECC and AEROVAULT-V3-SPEC v4 note. |
...
AeroAgent (50+ tools) operates under backend-enforced security controls:
