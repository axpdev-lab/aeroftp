...
pub fn import_s3cmd(path: &Path) -> Result<S3cmdImportResult, String> {
    let content =
        std::fs::read_to_string(path).map_err(|e| format!("read s3cmd config: {e}"))?;
...
pub fn export_s3cmd(
    ...
    let server = servers
        .iter()
        .find(|s| s.protocol.as_deref() == Some("s3"));
...
"#;
    ...
    fn test_parse_s3cmd_custom_endpoint() {
        ...
        assert_eq!(obj.get("region").and_then(|v| v.as_str()), Some("eu-west-1"));
    ...
    fn test_default_config_path_env_override() {
        ...
        let dir = std::env::temp_dir()
            .join(format!("s3cmd-cfg-{}", &crate::bridge_shared::uuid_v4()[..8]));
