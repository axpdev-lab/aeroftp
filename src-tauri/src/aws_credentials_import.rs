...
pub fn import_aws_credentials_with_config(
    ...
    let cred = std::fs::read_to_string(cred_path)
        .map_err(|e| format!("read aws credentials: {e}"))?;
    ...
    for name in names {
        ...
        let (host, port, provider_id) = match kv
            .get("endpoint_url")
            .filter(|s| !s.is_empty())
        {
            Some(ep) => {
...
pub fn export_aws_credentials(
    ...
    for server in servers {
        ...
        let endpoint = if !server.host.is_empty() {
            Some(if server.host.starts_with("http://")
                || server.host.starts_with("https://")
            {
                server.host.clone()
            } else {
...
"#;
    ...
    fn test_parse_fixture_three_profiles_one_skipped() {
        ...
        let result =
            import_aws_credentials_with_config(&cred, Some(&cfg)).expect("should import");
    ...
    fn test_secret_policy_plain_credential_populated() {
        ...
        let result =
            import_aws_credentials_with_config(&cred, None).expect("should import");
        std::fs::remove_file(&cred).ok();
    ...
    fn test_roundtrip_import_export_import_idempotent() {
        ...
        let first =
            import_aws_credentials(&cred).expect("first import");
        ...
        let n = export_aws_credentials(&export_servers, &passwords, &out)
            .expect("export");
        assert_eq!(n, 2);
