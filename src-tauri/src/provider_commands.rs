                ...
                match provider.list(remote_dir).await {
                    Ok(entries) => {
                        for entry in entries.into_iter().filter(|entry| !entry.is_dir) {
                            ...
                            let modified =
                                entry.modified.as_deref().and_then(crate::parse_remote_datetime);
...
pub async fn provider_compare_directories(
    ...
    if list_model.is_clone_pool() {
        ...
        for entry in remote_entries {
            ...
            let modified = entry.mtime.as_deref().and_then(crate::parse_remote_datetime);
...
> {
    ...
    while let Some(current_dir) = dirs_to_process.pop() {
        ...
        for entry in entries {
            ...
            let modified = entry.modified.as_deref().and_then(crate::parse_remote_datetime);
