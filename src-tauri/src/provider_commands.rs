...
mod tests {
    ...
    /// A real vault UUID is an explicit selection. Even if this connection has
    /// cached provider-overlay keys of the same kind, those keys must not
    /// shadow the selected standalone vault and normalize the listing with the
    /// wrong material.
    #[tokio::test]
    async fn compare_real_vault_uuid_outranks_same_kind_overlay_cache() {
        ...
        match &resolved {
            crate::crypt_overlay_provider::OverlayKeys::Rclone(keys) => {
                assert_eq!(keys.name_key, expected_name_key);
            }
