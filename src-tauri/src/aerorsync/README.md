...
4. **Scope funzionale**: single-file delta accelerator, non sostituto completo di rsync. Fuori scope: recursive tree sync, hardlink end-to-end, xattrs, ACL, `--inplace`, `--append`, `--delete*`, `--partial-dir`. Supportati: symlink end-to-end su Unix (Y-RSC.4: lstat della sorgente con `S_IFLNK` + target, ricreazione atomica lato receiver, `-l` preserve-links gia' presente nella flag string catturata, fail-closed su target non-Unix), `--mkpath` (remote parent creation) e un analogo `--sparse` opt-in sul local delta path (hole-punched writes, output byte-identical).
