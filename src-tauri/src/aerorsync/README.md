...
- **Profilo metadata di prodotto**: il comando integrato invia il literal `--server -ltprcze.iLsfxCIvu` (oppure la variante con `A` su Linux e `X` su Unix quando quei metadata sono attivi). Non richiede e non emette owner/group/devices (`-o -g -D`). I literal storici `-logDtp...` restano solo per gli oracle di capture.
