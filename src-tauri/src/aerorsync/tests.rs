//! Tests for the aerorsync native rsync module.
//!
//! Local, deterministic, no real SSH. Cover remote-command golden pins,
//! engine_adapter protocol bridges, mock transport basics, cancel handles,
//! and real-wire frozen-transcript oracles.
//!
//! Y-RSC.8 removed the RSNP SessionDriver / frame-codec / planner / session
//! unit suite with the legacy stack.
//!
...
use crate::aerorsync::engine_protocol_types::{DeltaInstruction, SignatureBlock};
...
use crate::aerorsync::fixtures::{
    ...
    DOWNLOAD_REMOTE_COMMAND, REAL_RSYNC_FROZEN_TRANSCRIPT_REL, REAL_RSYNC_LANE_PORT,
    UPLOAD_REMOTE_COMMAND,
};
...
use crate::aerorsync::types::{AerorsyncErrorKind, ProtocolVersion, SessionRole};
...
fn app_import_budget_matches_the_documented_inventory() {
    ...
    const DOCUMENTED: [(&str, &str, usize); 6] = [
