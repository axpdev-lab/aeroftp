...
pub fn import_restic_repo_url(
    ...
    if protocol == "s3" {
        if let Some(bucket) = initial_path.clone() {
            ...
            opts.insert("bucket".to_string(), serde_json::Value::String(bucket));
        }
    }
...
pub fn import_restic_from_env(
    ...
    Err(
        "no Restic repository: set RESTIC_REPOSITORY, RESTIC_REPOSITORY_FILE, or pass --repo"
            .to_string(),
    )
}
...
pub fn import_restic(path: &Path) -> Result<ResticImportResult, String> {
    let content =
        std::fs::read_to_string(path).map_err(|e| format!("read RESTIC_REPOSITORY_FILE: {e}"))?;
    let url = content
        .lines()
        .map(str::trim)
        .find(|l| !l.is_empty())
...
mod tests {
    ...
    fn import_s3_with_injected_aws_creds_populates_credential() {
        ...
        let r = import_restic_repo_url("s3:s3.amazonaws.com/bucket/restic", &env).expect("import");
        ...
        assert_eq!(
            opts.get("repo_kind").and_then(|v| v.as_str()),
            Some("restic")
        );
    }
    ...
    fn import_s3_without_creds_skips_with_reason() {
        ...
        assert_eq!(
            r.skipped[0].reason,
            "restic creds are runtime env, re-enter"
        );
    }
