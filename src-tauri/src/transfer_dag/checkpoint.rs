...
impl MultipartCheckpoint {
    ...
    fn transition(&mut self, next: CheckpointStatus) -> Result<(), String> {
        let legal = matches!(
            (self.status, next),
            (CheckpointStatus::Prepared, CheckpointStatus::Transferring)
                | (
                ...
                | (CheckpointStatus::Prepared, CheckpointStatus::Failed)
                | (CheckpointStatus::Transferring, CheckpointStatus::Failed)
                | (CheckpointStatus::PayloadComplete, CheckpointStatus::Failed)
                | (CheckpointStatus::Failed, CheckpointStatus::Transferring)
...
fn migrate(record: &mut MultipartCheckpoint) -> Result<(), String> {
    match record.schema_version {
        ...
        1 => {
            ...
            // visible completion, therefore a legacy terminal is safely
            // retried as payload-complete and only v2 can write Committed.
