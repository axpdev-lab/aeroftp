...
pub fn caveat_text(token: &str) -> &str {
    match token {
        S3_ETAG => "The ETag is the object MD5 only for single-part, non-SSE-KMS objects. For a multipart or KMS-encrypted object the digest is omitted rather than guessed.",
